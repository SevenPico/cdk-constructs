# cdk-construct-s3-log-storage — Disabled Example

Demonstrates the `enabled: false` pattern for `@sevenpico/cdk-construct-s3-log-storage`.

When a disabled context is passed, the construct skips all resource creation —
mirroring Terraform's `count = 0` behavior. No S3 bucket, no bucket policy, no
SQS queue is synthesized.
