# cdk-construct-lambda-error-notification — Minimal Example

Demonstrates `@sevenpico/cdk-construct-lambda-error-notification` with required props only.

## Configured Features

- **Context-derived naming** — DLQ, alarms, and pipe named from context
- **Default alarm thresholds** — 1 datapoint, 5 evaluation periods
- **Default pipe input template** — `<$.requestPayload>`
